...
hipDataType hipblasMMWrapper::getHiblasLtDataType(hipblasDatatype_t data_type)
{
                           ...
                           float             f_beta)
{
    ...
    bool using_cublasLt      = (Atype_ == HIPBLAS_R_16F) ? true : false;
    int  batch_count         = 1;
    ...
    if (using_cublasLt) {
        ...
        hipblasLtMatrixLayoutCreate(&Adesc, getHiblasLtDataType(Atype_),
            transa == HIPBLAS_OP_N ? m : k, transa == HIPBLAS_OP_N ? k : m, lda);
        hipblasLtMatrixLayoutCreate(&Bdesc, getHiblasLtDataType(Btype_),
            transb == HIPBLAS_OP_N ? k : n, transb == HIPBLAS_OP_N ? n : k, ldb);
        hipblasLtMatrixLayoutCreate(&Cdesc, getHiblasLtDataType(Ctype_),
            m, n, ldc);
        ...
        if (findAlgo) {
            if (info.workspaceSize > workspaceSize) {
                ...
                hipblaslt_ext::getAlgosFromIndex(cublaslt_handle_, algoIndex, heuristicResult);
            }
        }

        hipblasLtMatmul(cublaslt_handle_,
                       operationDesc,
                       alpha,
                       A,
                       Adesc,
                           ...
                           const int         ldc)
{
    ...
    cublasLtMatrixLayoutCreate(&Adesc, Atype, (transa == HIPBLAS_OP_N) ? m : k, (transa == HIPBLAS_OP_N) ? k : m, lda);
    cublasLtMatrixLayoutCreate(&Bdesc, Btype, (transb == HIPBLAS_OP_N) ? k : n, (transb == HIPBLAS_OP_N) ? n : k, ldb);
    cublasLtMatrixLayoutCreate(&Cdesc, Ctype, m, n, ldc);

    cublasLtMatmulDescCreate(&operationDesc, computeType, scaleType);
    cublasLtMatmulDescSetAttribute(operationDesc, CUBLASLT_MATMUL_DESC_TRANSA, &transa, sizeof(hipblasOperation_t));
                             ...
                             void*             C)
{
    ...
    hipsparseOperation_t opA = (transa == HIPBLAS_OP_N) ? HIPSPARSE_OPERATION_NON_TRANSPOSE : HIPSPARSE_OPERATION_TRANSPOSE;
    hipsparseOperation_t opB = (transb == HIPBLAS_OP_N) ? HIPSPARSE_OPERATION_NON_TRANSPOSE : HIPSPARSE_OPERATION_TRANSPOSE;
...
hipblasMMWrapper::MatrixLayout hipblasMMWrapper::createMatrixLayout(cublasLtMatrixLayout_t Mdesc)
{
    size_t       returnSize;
