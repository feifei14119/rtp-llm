...
template <typename ActType, WeightOnlyQuantType QType, typename Arch>
struct WeightLayoutDetails;

template <typename ActType>
struct WeightLayoutDetails<ActType, WeightOnlyQuantType::Int4b, cutlass::arch::Sm75>
{
...
template <typename ActType>
struct WeightLayoutDetails<ActType, WeightOnlyQuantType::Int8b, cutlass::arch::Sm75>
{
...
template <typename ActType>
struct WeightLayoutDetails<ActType, WeightOnlyQuantType::Int8b, cutlass::arch::Sm70>
{
    static constexpr int kElemBits = 8;
    ...
    // corresponding address in shared memory
    template <int Num, int WarpSize>
    __device__ __forceinline__ static void sync(float* res, float (*sm)[Num])
    {
#pragma unroll
        for (int i = 0; i < Num; ++i)
        {
            res[i] += __shfl_xor_sync(~0, res[i], 16);
            res[i] += __shfl_xor_sync(~0, res[i], 8);
            res[i] += __shfl_xor_sync(~0, res[i], 4);
            res[i] += __shfl_xor_sync(~0, res[i], 2);
            res[i] += __shfl_xor_sync(~0, res[i], 1);
        }
        __syncthreads();
...
{
    using Layout = WeightLayoutDetails<ActType, QType, Arch>;
...
template <typename ActType, WeightOnlyQuantType QType, typename Arch, typename WeightOnlyFlag, bool Zero, int BlockSize>
struct WeightOnlyScaleLoader
{
    using ElemType = ActType;
    using Details = WeightOnlyKernelDetails<ActType, QType, Arch>;
...
public:
    ...
    __device__ __forceinline__ void load_vec(ElemType* scale, ElemType* zero, int nid)
    {
        int offset = nid * Details::kInterleave;
        if constexpr (kIsFineGrained)
        {
        ...
        *reinterpret_cast<uint4*>(scale) = *reinterpret_cast<uint4*>(const_cast<ElemType*>(_scales + offset));
        if constexpr (Zero) {
            *reinterpret_cast<uint4*>(zero) = *reinterpret_cast<uint4*>(const_cast<ElemType*>(_zeros + offset));
        }
        else {
            *reinterpret_cast<uint4*>(zero) = make_uint4(0, 0, 0, 0);
        }
    }
    ...
    const ActType* bias, ActType* out, const int n, const int k)
{
    ...
    using ScaleLoader = WeightOnlyScaleLoader<ActType, QType, Arch, WeightOnlyFlag, Zero, BlockSize>;
    ...
    qweight += n_start_id;
    ScaleLoader scale_loader(scales, zeros, n_start_id, n);
...
#pragma unroll
    ...
    constexpr int ScaleNumPerAccess = sizeof(uint4) / sizeof(ActType);
    constexpr int ScaleAccessNum = NPerBlock / ScaleNumPerAccess;
    for (int i =0; i < ScaleAccessNum; i++){
        scale_loader.load_vec(scale + i * ScaleNumPerAccess, zero+ i*ScaleNumPerAccess, i * ScaleNumPerAccess);
    }
    constexpr int kWeightAccessNum = NPerBlock / Details::kElemsPerThread;

    int local_k = tid;

    // ping-pong buffer for pipeline
    uint8_t weights_quantized[NPerBlock * 2];
#pragma unroll
    for (int j = 0; j < kWeightAccessNum; j++) {
        load<AccType>(weights_quantized + j * Details::kElemsPerThread,
                      qweight + (local_k * n) + j * Details::kElemsPerThread);
    }

#pragma unroll 1
    for (; local_k < k - BlockSize * 2; local_k += BlockSize * 2) {
        ActType weights_vec[NPerBlock];
        ActType in_v[Batch];
...
#pragma unroll
            for (int b = 0; b < Batch; b++) {
                in_v[b] = in[b * k + local_k + buffer_idx * BlockSize];
            }

#pragma unroll
            for (int j = 0; j < kWeightAccessNum; j++) {
                load<AccType>(weights_quantized + ((buffer_idx + 1) % 2 * NPerBlock) + j * Details::kElemsPerThread,
                              qweight + ((local_k  + BlockSize + buffer_idx * BlockSize) * n) + j * Details::kElemsPerThread);
            }
#pragma unroll
            for (int i = 0; i < Details::kConvertIters * kWeightAccessNum; ++i) {
                assign<CvtResType>(weights_vec + i * Details::kConvertCount,
                                   Converter::convert(*reinterpret_cast<CvtSrcType*>(
                                       weights_quantized + (buffer_idx % 2 * NPerBlock)
                                       + i * Details::kConvertCount / Details::kElemsPerByte)));
                const int offset = i * Details::kConvertCount;
            }
...
#pragma unroll
                for (int b = 0; b < Batch; b++) {
                    *reinterpret_cast<ActType2*>(accumulator + i + b * NPerBlock) =
                        __hfma2(v, ActTypeDetails<ActType>::to_vec2(in_v[b]), *reinterpret_cast<ActType2*>(accumulator + i + b * NPerBlock));
                }
            }
        }
    }

    // last store to buffer
    if (local_k < k - BlockSize)
    {
        ActType weights_vec[NPerBlock];
        ActType in_v[Batch];
...
#pragma unroll
        for (int b = 0; b < Batch; b++) {
            in_v[b] = in[b * k + local_k];
        }

#pragma unroll
            for (int j = 0; j < kWeightAccessNum; j++) {
                load<AccType>(weights_quantized + NPerBlock + j * Details::kElemsPerThread,
                              qweight + ((local_k  + BlockSize ) * n) + j * Details::kElemsPerThread);
            }
...
#pragma unroll
        for (int i = 0; i < Details::kConvertIters * kWeightAccessNum; ++i) {
            assign<CvtResType>(
                weights_vec + i * Details::kConvertCount,
                Converter::convert(*reinterpret_cast<CvtSrcType*>(
                    weights_quantized + NPerBlock + i * Details::kConvertCount / Details::kElemsPerByte)));
            const int offset = i * Details::kConvertCount;
        }
...
#pragma unroll
    ...
    for (int i = tid; i < Num; i += BlockSize)
    {
        ...
        out[n_start_id + nid + b * n] = static_cast<ActType>(ActOp<float>::apply(v));
    }
}
...
#pragma unroll
                for (int j = 0; j < Details::kShuffleStrided; ++j)
                {
                    ...
                    // register array
                    ActType2 v =
                        *reinterpret_cast<ActType2*>(weights_vec + i * Details::kShuffleBasicTile
...
#pragma unroll
                ...
                {
                    v = __hfma2(*reinterpret_cast<ActType2*>(weights_f16 + y), *reinterpret_cast<ActType2*>(in_v + y), v);
                }
                    ...
                    for (int y = 0; y < Details::kElemsPerThread; ++y)
                    {
                        *reinterpret_cast<ActType2*>(accumulator + b * NPerBlock + x * 2)
                            = __hfma2(*reinterpret_cast<ActType2*>(weights_f16 + y * NPerBlock + x * 2),
                                ActTypeDetails<ActType>::to_vec2(in_v[y]), *reinterpret_cast<ActType2*>(accumulator + b * NPerBlock + x * 2));
                    }
                }
            }
        }
...
{
    static constexpr int NPerBlock = 64;
        ...
        if (params.act_type == WeightOnlyActivationType::FP16) {
            ...
            weight_only_batched_gemv_sm70_wrapper<half, QType, Arch, WeightOnlyFlag, ActOp, Zero, Bias, NPerBlock, Batch, BlockSize>
                <<<grid, block, size, stream>>>(params.qweight,
                                                ...
                                                reinterpret_cast<const half*>(params.in),
                                                reinterpret_cast<const half*>(params.bias),
            ...
            weight_only_batched_gemv_sm70_wrapper<__nv_bfloat16, QType, Arch, WeightOnlyFlag, ActOp, Zero, Bias, NPerBlock, Batch, BlockSize>
                <<<grid, block, size, stream>>>(params.qweight,
                                                ...
                                                reinterpret_cast<const __nv_bfloat16*>(params.bias),
                                                reinterpret_cast<__nv_bfloat16*>(params.out),
                                                params.n,
                                                params.k);
        }
#endif
...
{
    static constexpr int NPerBlock = 2;
