...
docker run --ulimit nofile=655350:655350 --privileged --cap-add SYS_ADMIN --device /dev/fuse --ulimit memlock=-1 --cpu-shares=15360 --cpu-quota=9600000 --cpu-period=100000 --memory=500000m -d --net=host --name $CONTAINER_NAME -v /home/feiw/ali:/ali -v $HOME:/home/$USER -v $CONTAINER_DIR/initContainer.sh:$CONTAINER_DIR/initContainer.sh --device=/dev/kfd --device=/dev/dri --group-add video $IMAGE /sbin/init 1> /dev/null
...
docker exec $CONTAINER_NAME /bin/bash -c "sudo ln -s /opt/conda310/bin/python3 /usr/bin/python && sudo ldconfig"
